...
echo "<form action='pdo.php' method='post'>";$nextMonth = ((date(m)+1) < 10 ) ? "0".(date(m)+1) : (date(m)+1);
...
$lastMonthName = date(F, mktime(0, 0, 0, $lastMonth, 30, 1997));$x = date(y) - 1;
$X = date(Y) - 1;
$y = date(y);
$Y = date(Y);
$z = date(y) + 1;
...
$conn->close();
	...
	echo "<center><input class='btn' type='submit' name='formSubmit' value='SUBMIT' style='width: 100%; background-color: #A00; color: #DADADA; font-weight: bold; width: 50%;' /></center></form>";
