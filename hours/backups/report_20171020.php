...
echo "<p>Total reported hours for 1718: ";

$query06 = "SELECT Hours FROM `HOURS` RIGHT JOIN `MEMBERS` ON `MEMBERS`.VolID = `HOURS`.VolunteerID JOIN `ACTIVITIES` ON `ACTIVITIES`.ActID = `HOURS`.Activity WHERE Month LIKE '1718%' ORDER BY UNIX_TIMESTAMP(TimeStamp) DESC";

$result06 = $conn -> query($query06);

$sumHours1718 = 0;

while ( $row = $result06 -> fetch_assoc() ) { $sumHours1718 += $row['Hours']; }
